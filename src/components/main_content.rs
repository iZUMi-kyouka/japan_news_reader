...
use crate::utils::nhkreader::{get_metadata, get_metadata_from_memory, update_metadata, update_metadata_in_memory};
    ...
    // Update icon highlight
    {
        ...
        use_effect(move || {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                ...
                element.set_attribute("active", "true");
            });
    ...
    let result = {
        ...
        use_future(|| async move {
            ...
            if let Some(cookie) = get_all_cookie() {
                // log!(&format!("found cookie: {:#?}", &cookie));
                cookie_expiry = cookie.get("cacheExpiry").map(|s| s.to_owned());
            }

            let cur_time = Utc::now().timestamp();
            // log!(&format!("current time: {}", cur_time));
            if let Some(entry) = cookie_expiry.as_ref() {
                // log!(&format!("cache expiring in:  {} ", entry));
                if entry.parse::<i64>().unwrap() < cur_time {
                    // log!("cache has expired. invalidating cache...");
                    cookie_expiry = None;
                } else {
                    // log!("cache is still valid. now checking cache in both memory and local storage...");
                }
            }

            // log!(&format!("cookie expiry: {:?}", &cookie_expiry));
            // Cache is not yet expired. Try retrieving from memory and local storage
            if let Some(_) = cookie_expiry {
...
pub fn JapanNewsContent(props: &JapanNewsProp) -> HtmlResult {
    ...
    // Update icon highlight
    {
        ...
        use_effect(move || {
            log!("use effect ran.");
            cx.dispatch(AppCtx {
                active_news: NewsCategory::Japan { page },
                ..(*cx).clone()
            });
    ...
    let result = {
        ...
        use_future(|| async move {
            ...
            if let Some(ref entry) = cookie_expiry {
                let cur_time = Utc::now().timestamp();
                if entry.parse::<i64>().unwrap() < cur_time {
                    // log!(&format!("cache expiring in :  {} -- current time is : {}", entry, cur_time));
                    cookie_expiry = None;
                }
            }
